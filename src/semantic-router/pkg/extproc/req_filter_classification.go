...
func (r *OpenAIRouter) selectModelFromCandidates(modelRefs []config.ModelRef, decisionName string, query string, algorithm *config.AlgorithmConfig, categoryName string, candidateIterations []config.CandidateIterationConfig, ctx *RequestContext) (*config.ModelRef, string) {
	if len(modelRefs) == 0 {
		return nil, ""
	}
